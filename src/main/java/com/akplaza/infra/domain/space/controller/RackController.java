...
public class RackController {
    ...
    @Operation(summary = "랙 정보 수정", description = "특정 장소(Location)의 랙 정보를 업데이트합니다.")
    @PutMapping("/{id}")
    public ResponseEntity<Long> updateRack(@Valid @PathVariable Long id, @Valid @RequestBody RackRequest request) {
        log.info("Rack API: 수정 요청 - RackNo: {}", request.getRackNo());
        Long rackId = rackService.updateRack(id, request);
        return ResponseEntity.status(HttpStatus.CREATED).body(rackId);
    }
